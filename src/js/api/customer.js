...
const customersApi = {
    ...
    updateMainImage: async (id, idImage) => {
        ...
        if ( token ) return apiRequest('PUT', BASE_URL + `/api/customer/${id}/set-main-image/${idImage}?access-token=${token}`)
    },
    ...
    getEquipmentCustomer: async (id) => {
        ...
        if ( token ) return apiRequest('GET', BASE_URL + `/api/customer/${id}/equipment?access-token=${token}`, {}, {}) 
    },

    getNetwork: async (id) => {
        const token = getToken()

        if ( token ) return apiRequest('GET', BASE_URL + `/api/customer/${id}/network?access-token=${token}`, {}, {})
    },

    getConstruct: async (id) => {
        const token = getToken()

        if ( token ) return apiRequest('GET', BASE_URL + `/api/customer/${id}/construct?access-token=${token}`, {}, {})
    },

    deleteAll: async (id, typeId) => {
        const token = getToken()

        if ( token ) return apiRequest('GET', BASE_URL + `/api/customer/${id}/image/${typeId}/delete-all?access-token=${token}`, {}, {})
    }
}
